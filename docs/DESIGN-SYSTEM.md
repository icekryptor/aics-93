...
| Ширина КП (документный вид) | образец — `kp/bigsnt/doc` | лист `max-w-[1140px]`; проза, цитаты, callout'ы, FAQ и источники — колонкой `max-w-[50rem]`; интерактив (экономика, Гант, Директ), карточки фактов и кейсов (`lg:grid-cols-4`) — на всю ширину листа. В лист 900px интерактив не зажимать — «всё в кучу» (фидбек 13.09.2026) |
...
| `KpChapterNav` | `components/kp/KpChapterNav.tsx` | липкая навигация по разделам КП слева сверху (мотив `ChapterNav` кейсов), `hidden xl:block`. Подписи разделов видны всегда на стеклянной подложке (не по ховеру — «полоса номеров непонятно что и зачем», фидбек 13.09.2026); шеллы КП-страниц отдают колонке левое поле `xl:pl-[220px]` до ширины, где она влезает в поля (`min-[1600px]`/`min-[1720px]` для 6xl/7xl, `min-[1400px]` для листа 900). Активный раздел считается **по геометрии** (последний, чей верх прошёл 30% вьюпорта), а не через IntersectionObserver: разделы высокие, в IO-полосу попадают сразу несколько и побеждал произвольный. У низа страницы последний раздел подсвечивается принудительно — короткую концевую секцию иначе не поймать |
