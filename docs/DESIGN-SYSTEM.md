...
- **Иллюстрации статей** — генеративные (Higgsfield `nano_banana_pro`, 16:9): тёмная сцена #14101d, wireframe-объекты, свечение строго #9747ff/#5fd9f5, blueprint-сетка, без текста в кадре. Файлы `public/assets/blog/*.webp` (sharp q88, ~1376×768; PNG-оригиналы в репо не кладём). В body — отдельным абзацем `![alt|подпись](/assets/blog/x.webp)`; подпись lowercase «рис. NN · …»; квадратные скобки в alt/подписи запрещены — ломают парсер MarkdownLite.
