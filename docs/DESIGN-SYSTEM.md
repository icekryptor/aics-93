...
- **Единственное исключение — `.kp-doc`** (документный вид КП, scoped-класс в globals.css): заголовки **Playfair Display**, body **Lora 18px / 140%**. Причина: КП читают целиком и печатают, а нетехническая аудитория на серифе с крупным интерлиньяжем устаёт заметно меньше. Внутри `.kp-doc` правило «цифры — Unbounded» остаётся в силе. Новых исключений не добавлять без явного решения Василия.
...
| `EngineBlock` | `components/EngineBlock.tsx` | «как работает ИИ-движок» (главная, ACT III): терминал-карточка engine.log (`.hud normal-case`, шапка с 3 точками), роли-чипы «1 = 4», честная плашка с бордером `--color-signal-cool`; тайминги фаз = Гант лендинга сайтов |
...
| Шаблон КП | `app/(services)/kp/[client]/page.tsx` (образец — martox) | КП-лендинг клиенту: noindex + вне sitemap; кликабельное содержание якорями; структура ТЗ с подсвеченными рекомендациями (рамка `--color-signal-cool` + бейдж «рекомендуем добавить» цвета циан/#0b1e33); цены с зачёркнутым фуллпрайсом + бейдж скидки; `GanttToggle` (client, полноширинный Гант с переключателем вариантов); работы — реальные скрины сайтов (jpg 1200px, `assets/kp/works/`); финальный CTA — квиз-решение `QuizInline` + `kpDecisionSteps()` (образец — gagarin; в martox остался старый CTA: TG + телефон). **Тон подстраиваем под аудиторию**: для нетехнических клиентов (образец — gagarin, преподаватели-гуманитарии) технический словарь запрещён — не «база данных», «развёртывание», «LMS», «SEO», а «платформа открылась на вашем адресе», «статьи помогают находиться в Яндексе»; body в таких КП 16.5px вместо 15px ради читаемости. **Два вида отображения** (образцы — gagarin и broker): студийный `/kp/<client>` (тёмная сцена, канон) и документный `/kp/<client>/doc` (светлый лист, сериф) — переключатель `KpViewSwitch` в шапке обоих; контент общий в `lib/kp/<client>.ts`, дублировать текст между видами запрещено |
| `RouteMap` | `components/kp/RouteMap.tsx` | схема пути товара (сквозной мотив КП/сайтов логистики): ноды-точки со свечением + линия-градиент фиолет→циан, горизонтальный скролл на мобиле |
| `RevenueBar` | `components/kp/RevenueBar.tsx` | доходная часть КП: один бар месячного дохода (сумма — `font-display`) + сегменты, каждый объяснён под баром (цвет-плашка, сумма, откуда деньги). Порядок цветов сегментов: `--color-signal` → `#5fd9f5` → `#c5ff44` → `--color-signal-cool`; `ink` сегмента задаётся вручную под контраст. `tone="paper"` — версия под документный лист (сегменты темнее: `#6b34c9`/`#1e8a7a`/`#c98a12`/`#8a5bb8`, белый текст) |
| `BrokerBooks` | `components/kp/BrokerBooks.tsx` | схема разницы A-book и B-book для КП брокера: сделка сквозь брокера к ликвидности против сделки, оставшейся внутри; снизу — гибрид-классификатор. Один SVG на оба вида КП, палитра через `tone` dark/paper |
| `BrokerPlan` | `components/kp/BrokerPlan.tsx` | план работ КП брокера двумя дорожками сразу (трек клиента: юрлицо/банк/наём — и трек студии: майлстоуны). Переключателя между треками нет намеренно: смысл схемы в том, что критический путь — верхняя дорожка, а не разработка. Бары — div'ы по сетке месяцев, вертикали — гейты |
| `BrokerUnitEconomics` | `components/kp/BrokerUnitEconomics.tsx` | живая юнит-экономика брокера (client): 10 ползунков + 3 пресета сценариев, справа бар распределения дохода с клиента и построчная структура. Ручное движение ползунка снимает подсветку пресета. `tone` dark/paper |
| `BigsntUnitEconomics` | `components/kp/BigsntUnitEconomics.tsx` | экономика одного заказа для КП BIGSNT (client): 14 ползунков в трёх группах (товар / площадка / свой сайт) + пресеты WB/Ozon, справа четыре плашки, бар «каждые 100 ₽ покупателя» для обоих каналов, построчная структура двумя колонками, окупаемость трёх объёмов в переведённых повторных заказах. Модель WB: комиссия и ДРР — от цены продавца до СПП, СПП финансирует площадка, покупатель платит одинаково в обоих каналах. Экспортирует `computeUnit()` — им же пользуется `BigsntDirect`. `tone` dark/paper |
| `BigsntDirect` | `components/kp/BigsntDirect.tsx` | тот же бюджет Директа при двух конверсиях (сейчас / после запуска): 4 ползунка → клики, заказы, стоимость заказа, дополнительная прибыль по экономике сайта из `computeUnit` на значениях по умолчанию. `tone` dark/paper |
| `BigsntPlan` | `components/kp/BigsntPlan.tsx` | Гант плана работ по пяти этапам производственного пайплайна студии (исследование → прототип → дизайн-система → сборка → настройка и передача) с переключателем объёма 01/02/03: базовые полосы растягиваются коэффициентом, полосы доп. объёма добавляются (`planFor()` в `lib/kp/bigsnt.ts`). Полосы окрашены по типу работы (думаем / рисуем и пишем / собираем / проверяем), ромб — точка решения клиента, пунктир — гейт. Шкала недель сверху, сетка дней. `tone` dark/paper |
| Скрины аудита в КП | `public/assets/kp/<client>/*.jpg` (образец — bigsnt) | снимки живого сайта клиента к пунктам аудита: мобильные 375×812 @1.5 (≈560px), десктоп 1280 @0.75 (960px), jpg q82; снимаются через html2canvas в browser-pane (cloud-контейнер до клиентских сайтов не достаёт), в подписи — дата снимка. Мобильный скрин в карточке аудита показывается узкой колонкой 150px, десктопный — на всю ширину третьей колонки |
| Ширина КП (студийный вид) | образец — `kp/bigsnt` | каркас `max-w-7xl`, проза и лиды `max-w-4xl`, h1/оглавление/FAQ/источники `max-w-5xl`, интерактивные блоки — на всю ширину каркаса. Не зажимать текст в 3xl: на широких экранах читается как «узкий фрейм» (фидбек Василия 12.09.2026) |
| Ширина КП (документный вид) | образец — `kp/bigsnt` | лист `max-w-[1140px]`; проза, цитаты, callout'ы, FAQ и источники — колонкой `max-w-[50rem]`; интерактив (экономика, Гант, Директ), карточки фактов и кейсов (`lg:grid-cols-4`) — на всю ширину листа. В лист 900px интерактив не зажимать — «всё в кучу» (фидбек 13.09.2026) |
| КП bigsnt — только документ | `kp/bigsnt` | единственный вид КП — светлый лист-сериф (студийный тёмный убран, /kp/bigsnt/doc → 307). Шапка: один кэпшн «кп для bigsnt», h1 = суть оффера, два объёма работ. Аудит: крупные скрины с лайтбоксом (`AuditShotLightbox`: превью обрезано по высоте, полный кадр + описание по клику). Экономика: блок «товар» → два независимых калькулятора (маркетплейс/сайт) → панель «Сравнение каналов» с KPI, шкалой 100 ₽ и двумя SVG-графиками (год клиента, окупаемость) |
| Стенд мозга хиро | `/panel/hero` + `components/panel/HeroStand.tsx` | настроечный стенд визуала мозга (паттерн evo brain-lab): ползунки живут поверх `lib/brain-config.json` (прод-версия, попадает в бандл на сборке), событие `aics:brainconfig` пересобирает BrainGL; «запушить» → `/api/brain-config` коммитит JSON в main через GitHub API (env `GITHUB_TOKEN`, contents:write) → автодеплой. Доступ — кука панели |
| `KpViewSwitch` | `components/kp/KpViewSwitch.tsx` | переключатель двух видов КП (студийный ↔ документ), `tone` dark/paper; ставится в шапку обоих видов |
| `KpChapterNav` | `components/kp/KpChapterNav.tsx` | липкая навигация по разделам КП слева сверху (мотив `ChapterNav` кейсов), `hidden xl:block`. Чистый текст без номеров, тиков и рамок (фидбек 13.09.2026), активный раздел — цветом `--color-signal-2`; проп `visibleFrom` — готовые классы видимости, страница показывает колонку только там, где она влезает в поле рядом с контентом (bigsnt: `hidden min-[1560px]:block`, лист центрован). Шеллы broker/gagarin по-прежнему отдают колонке левое поле `xl:pl-[220px]`. Активный раздел считается **по геометрии** (последний, чей верх прошёл 30% вьюпорта), а не через IntersectionObserver: разделы высокие, в IO-полосу попадают сразу несколько и побеждал произвольный. У низа страницы последний раздел подсвечивается принудительно — короткую концевую секцию иначе не поймать |
| `KpIndexList` | `components/kp/KpIndexList.tsx` | содержимое приватного индекса готовых КП (`/kp`); вынесено из страницы, чтобы вёрстку можно было проверить временным маршрутом, не вводя пароль в форму |
| `KpCopyLink` | `components/kp/KpCopyLink.tsx` | «скопировать ссылку» на КП: абсолютный адрес из `location.origin` в момент клика (на превью-деплое ссылка его же), фолбэк-подпись при отказе clipboard |
| Реестр КП | `lib/kp/registry.ts` | собранные КП: клиент, цена, срок, статус, все виды. Источник для `/kp`; собрал КП — добавь запись, иначе он потеряется |
| `GagarinMark` | `components/kp/GagarinMark.tsx` | эмблема школы «Гагарин» (орбита + звезда), `tone` dark/paper — плейсхолдер до получения их логотипа |
| Квиз-финал КП | `lib/quiz.ts` → `kpDecisionSteps()` + `QuizInline` | **дефолт для всех КП**: финал — не «напишите в телеграм», а квиз-решение (выбор варианта → срок → доп. шаги КП → вопросы → связь). Отдельный воркер под каждое КП не заводим: `QuizInline` шлёт в `/api/lead`, оттуда Тихоном (`@aics93_tikhon_bot`) в общий поток заявок |
| Harness-панель | `app/(services)/panel/page.tsx` + `lib/panel.ts` | приватный реестр проектов (/panel, noindex, вне sitemap): пароль `PANEL_PASSWORD` (env) → httpOnly-cookie 30 дней (sha256-токен, смена пароля инвалидирует куки); карточки: id·клиент·статус·этап·дата·чипы-ссылки с точкой-индикатором по типу (`live` циан / `design` фиолет / `doc` серый); артефакты берутся из Continuum Harness (`harness.db`: projects, design_exports, factory_attachments) и выгружаются в `public/clients/*`; SalesEngine на главной припаркован (пояснение механики — EngineBlock/AiProcess) |
| Визуал КП/клиентских сайтов | — | референс-зона финтех/логистика (Mercury, Stripe Atlas, Flexport, Ramp): схемы и таблицы вместо стока, документальная фотография (склад, паллеты, контейнеры, muted-тона); ЗАПРЕЩЕНО: «американская мечта» (флаги, статуя Свободы, орлы, небоскрёбы, доллары), стартап-градиенты, 3D-иллюстрации «SaaS», неон-кино для документальных ниш |
...
- **Минус-фразы (не употреблять нигде — тексты, код, доки, промпты):** «конвейер» / «conveyor»
...
- **Скролл-зависимое (липкие навигации, IO-подсветка) проверять Playwright-ом.** В browser-pane скролл не двигается вообще (`window.scrollY` остаётся 0, замеры ширины дают 0), а headless со `#якорем` рендерит пустую страницу. И помни: у html глобально `scroll-behavior: smooth` — после `scrollTo` дожидайся остановки (опрос `window.scrollY` до совпадения), иначе замеришь середину анимации и получишь ложный баг.
