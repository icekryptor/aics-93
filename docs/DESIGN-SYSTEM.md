...
| Шаблон КП | `app/(services)/kp/[client]/page.tsx` (образец — martox) | КП-лендинг клиенту: noindex + вне sitemap; hero с фото-фоном (Higgsfield, тёмная фиолет/циан гамма) + кликабельное содержание якорями; структура ТЗ с подсвеченными рекомендациями (рамка `--color-signal-cool` + бейдж «рекомендуем добавить» цвета циан/#0b1e33); варианты реализации 2 карточки; Гант ×2 (паттерн ServiceDetail); работы — реальные скрины сайтов (jpg 1200px, `assets/kp/works/`); финальный CTA — TG + телефон, без формы |
