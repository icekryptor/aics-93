...
| `RevenueBar` | `components/kp/RevenueBar.tsx` | доходная часть КП: один бар месячного дохода (сумма — `font-display`) + сегменты, каждый объяснён под баром (цвет-плашка, сумма, откуда деньги). Порядок цветов сегментов: `--color-signal` → `#5fd9f5` → `#c5ff44` → `--color-signal-cool`; `ink` сегмента задаётся вручную под контраст |
