...
| Harness-панель | `app/(services)/panel/page.tsx` + `lib/panel.ts` | приватный реестр проектов (/panel, noindex, вне sitemap): пароль `PANEL_PASSWORD` (env) → httpOnly-cookie 30 дней (sha256-токен, смена пароля инвалидирует куки); карточки: id·клиент·статус·дата·чипы-ссылки; SalesEngine на главной припаркован (пояснение механики — ConveyorBlock/AiProcess) |
