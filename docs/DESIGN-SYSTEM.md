...
- **Иллюстрации статей** — генеративные (Higgsfield `nano_banana_pro`, 16:9, resolution 2k): изометрия ~30°, frosted-glass 3D-рендер — полупрозрачное матовое стекло, монохромный фиолет/лаванда/белый, неоновые PCB-дорожки, фон #2c2545; канон промпта и шаблон — `docs/ILLUSTRATIONS.md`. Плоский 2D-wireframe — нельзя (пройдено, примитивно). Файлы `public/assets/blog/*.webp` (sharp resize 1600 + q88; PNG-оригиналы в репо не кладём). В body — отдельным абзацем `![alt|подпись](/assets/blog/x.webp)`; подпись lowercase «рис. NN · …»; квадратные скобки в alt/подписи запрещены — ломают парсер MarkdownLite.
