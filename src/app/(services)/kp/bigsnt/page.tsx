...
import AuditShotLightbox from "@/components/kp/AuditShotLightbox";
...
import {
  KP_META, TOC, FACTS, TASK_PARAS, AUDIT_LEAD, AUDIT_QUOTE, AUDIT_ROWS,
...
/* КП для BIGSNT — единственный вид: светлый лист, сериф, под вдумчивое
   чтение и печать (студийный тёмный вид убран по фидбеку 13.09.2026).
   Стили — scoped-класс `.kp-doc` в globals.css; лист центрован, навигация
   глав показывается только там, где влезает в поле рядом с листом. */
...
export const metadata: Metadata = {
  title: "КП для BIGSNT — свой канал продаж",
  description: "Коммерческое предложение для BIGSNT: аудит сайта, экономика заказа, два объёма работ, план и стоимость.",
  ...
  return (
    <div className={`${playfair.variable} ${lora.variable} px-4 py-10 sm:px-6 sm:py-14`}>
      ...
      <article className="kp-doc mx-auto w-full max-w-[1140px] overflow-hidden rounded-[14px] px-6 py-12 shadow-[0_30px_80px_-30px_rgba(0,0,0,0.7)] sm:px-14 sm:py-16">
        {/* ---------- шапка: один кэпшн, суть оффера первой строкой ---------- */}
        <header className="border-b-2 pb-8" style={{ borderColor: "var(--paper-ink)" }}>
          <p className="doc-eyebrow">кп для bigsnt</p>
        ...
        {/* ---------- 01 ---------- */}
        <section id="task" className="scroll-mt-8 pt-12">
        ...
        <section id="audit" className="scroll-mt-8 pt-12">
          ...
          <div className="mt-7 grid gap-4">
            {AUDIT_ROWS.map((r) => (
              <div
                key={r.what}
                className={`doc-card grid gap-5 p-5 sm:p-6 ${r.shot && r.shot.ratio === "mobile" ? "sm:grid-cols-[1fr_250px]" : ""}`}
              >
                <div>
                  ...
                  <p className="mt-3 max-w-[46rem] text-[15.5px] leading-[1.55]">{r.what}</p>
                  <p className="doc-soft mt-2.5 max-w-[46rem] text-[15px] leading-[1.55]">{r.cost}</p>
                </div>
                {r.shot ? (
                  <div className={r.shot.ratio === "desktop" ? "max-w-[880px]" : "self-center"}>
                    <AuditShotLightbox shot={r.shot} />
                  </div>
                ) : null}
              </div>
            ))}
        ...
        {/* ---------- 04 ---------- */}
        <section id="direct" className="scroll-mt-8 pt-12">
        ...
        {/* ---------- 05 ---------- */}
        <section id="offer" className="scroll-mt-8 pt-12">
          <Rule>05 · два объёма работ</Rule>
          ...
          <div className="mt-7 grid gap-5 lg:grid-cols-2">
            {TIERS.map((t) => (
              <article key={t.key} className="doc-card flex flex-col p-5 sm:p-6" style={t.rec ? { borderColor: "var(--paper-accent)", borderWidth: 2 } : undefined}>
        ...
        {/* ---------- 06 ---------- */}
        <section id="plan" className="scroll-mt-8 pt-12">
        ...
        {/* ---------- 07 ---------- */}
        <section id="price" className="scroll-mt-8 pt-12">
          <Rule>07 · стоимость и условия</Rule>
          <h2 className="text-[1.7rem] leading-tight"><span className="font-display">320 / 500</span> тысяч рублей — по объёму, без сюрпризов</h2>
        ...
        {/* ---------- 08 ---------- */}
        <section id="studio" className="scroll-mt-8 pt-12">
